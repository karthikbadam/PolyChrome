...
PeerServer.prototype._initializeHTTP = function() {
  ...
  this._app.get('/:key/id', function(req, res, next) {
    ...
    var clientId = self._generateClientId(req.params.key);
    //Collect the ids
  ...
  // Retrieve guaranteed random ID.
  this._app.get('/:key/allClientIds', function(req, res, next) {
    res.contentType = 'text/html';
    res.send(connectedClientIds);
    console.log("get peer IDs");
    return next();
  });
...
PeerServer.prototype._removePeer = function(key, id) {
  ...
  //remove element from the clientId list
  var index = connectedClientIds.indexOf(id);
  if (index != -1) {
    connectedClientIds.splice(index, 1);
