...
app.get('/images/*', function (req, res) {
    ...
    request({ uri: currentPage.getbaseUrl() + req.url }, function (err, response, body) {
       if (err) {
            res.writeHead(400);
        } else {
            res.writeHead(200);
...
app.get('/*', function (req, res) {
    ...
    if (currentPage) {
        ...
        console.log("Requesting for "+currentPage.getbaseUrl() + req.url)
        request({
            uri: currentPage.getbaseUrl() + req.url
        }, function (err, response, body) {
            if (err) {
...
app.post('/loadUrl', function (req, res) {
    ...
    request({ uri: page.parseUrl() }, function (err, response, body1) {
        var isBlocked = 'No';
            ...
            scripts: ['http://code.jquery.com/jquery.js', 'http://localhost:3000/javascripts/peer.js', 'http://localhost:3000/javascripts/polychrome-accesspanel.js'],
                ...
                var $ = window.jQuery;
                  ...
                  $('script').each(function() {
                 	  ...
                 	  var url = currentPage.getbaseUrl()+link;
                 	  console.log(url);
                 	  $(this).attr('src', url);
                 	}
                 });	
                 ...
                 $('link').each(function () {
                    ...
                    if (css.indexOf(".com") == -1) {
                        var url = currentPage.getbaseUrl() + css;
                        console.log(url);
                        $(this).attr('href', url);
                    }
                });
                ...
                $('image').each(function () {
                    ...
                    if (image !== undefined && image.indexOf(".com") == -1) {
                        var url = currentPage.getbaseUrl() + image;
                        console.log(url);
                        $(this).attr('href', url);
                    }
                });
                ...
                $('a').each(function () {
                    ...
                    if (hyperlink !== undefined && hyperlink.indexOf(".com") == -1) {
                        var url = currentPage.getbaseUrl() + hyperlink;
                        console.log(url);
                        $(this).attr('href', url);
                    }
                });
