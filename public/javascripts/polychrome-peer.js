...
function Peer(id, options) {
  ...
  //Get ids of all other connected clients
  this._retrieveAllIds();
...
Peer.prototype._retrieveAllIds = function(cb) {  
  var self = this;
  try {
    var http = new XMLHttpRequest();
    var url = 'http://' + this._options.host + ':' + this._options.port + '/' + this._options.key + '/allClientIds';
    var queryString = '?ts=' + new Date().getTime() + '' + Math.random();
    url += queryString;
    // If there's no ID we need to wait for one before trying to init socket.
    http.open('get', url, true);
    ...
    this._abort('server-error', 'Could not get the ID list from the server');
  }
};
...
Peer.prototype._handleServerJSONMessage = function(message) {
  ...
  switch (message.type) {
    case 'OPEN':
      this._processQueue();
      console.log("Peers are "+ this.clientIds);
      this.emit('open', this.id, this.clientIds);
      break;
    case 'ERROR':
